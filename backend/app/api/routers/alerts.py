...
async def ack_alert(alert_id: int, payload: AcknowledgeRequest, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    obj = await alert_engine.acknowledge(db, alert_id, payload.actor)
    if not obj:
